...
# بيانات الـ Service Account كاملة ومصلحة
...
def add_task_to_sheet(task_name):
    try:
        ...
        # ترتيب الأعمدة حسب صورة الشيت (Tasks)
        # A: المهمة | B: الأولوية | C: الحالة | D: تاريخ الإضافة | E: وقت البداية | F: وقت الانتهاء | G: وقت التنفيذ المتوقع | H: الديدلاين | I: ملاحظات
        row_data = [
            task_name,          # العمود A
            "متوسطة",            # العمود B (افتراضي)
            "قيد التنفيذ",       # العمود C (تلقائي)
            now_date,           # العمود D
            now_time,           # العمود E
            "", "", "",         # F, G, H (فارغ)
            "تمت الإضافة عبر البوت" # العمود I
        ]
