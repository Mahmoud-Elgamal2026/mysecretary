...
def get_sheet_service():
    ...
    # الكود دلوقتى هيتأكد إن الملف جواه بيانات صح مش كود بايثون
    try:
        creds = service_account.Credentials.from_service_account_file(
            auth_file, scopes=['https://www.googleapis.com/auth/spreadsheets'])
        return build('sheets', 'v4', credentials=creds, cache_discovery=False).spreadsheets()
    except Exception as e:
        raise ValueError(f"مشكلة في ملف credentials.json: تأكد إنه مش جواه كود البوت! الخطأ: {e}")

def setup_all_sheets_sync():
    try:
        service = get_sheet_service()
        # الـ 8 أقسام اللي طلبتهم يا محمود
        required = {
        ...
        return "✅ مبروك يا حودة! الـ 8 تابات بقوا جاهزين في الشيت."
    except Exception as e:
        return f"❌ خطأ: {str(e)}"
...
async def handle_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.answer()
    if query.data == 'setup':
        await query.edit_message_text("⏳ جاري تهيئة الـ 8 تابات...")
