...
# بيانات الـ Service Account اللي بعتها يا حودة
...
# دالة ذكية لتسجيل المهمة في الأعمدة المحددة في صورتك
def add_task_to_sheet(task_name):
    try:
        ...
        # الترتيب بناءً على الصورة:
        # A: المهمة | B: (فارغ للأولوية) | C: الحالة | D: تاريخ الإضافة | E: وقت البداية | F: وقت الانتهاء | G: وقت التنفيذ المتوقع | H: الديدلاين | I: ملاحظات
        row_data = [
            task_name,          # المهمة (Column A)
            "متوسطة",            # الأولوية (Column B) - افتراضي
            "قيد التنفيذ",       # الحالة (Column C)
            now_date,           # تاريخ الإضافة (Column D)
            now_time,           # وقت البداية (Column E)
            "",                 # وقت الانتهاء (Column F)
            "",                 # وقت التنفيذ (Column G)
            "",                 # الديدلاين (Column H)
            "أضيفت من البوت"     # ملاحظات (Column I)
        ]
        ...
        return f"❌ حصلت مشكلة وأنا بعدل الشيت: {str(e)}"

async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("👔 سكرتير محمود المصري جاهز.\nابعت لي المهمة اللي عايز تسجلها في الشيت:")
