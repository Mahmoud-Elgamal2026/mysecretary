...
# المفتاح السري مصلح يدوياً لضمان عدم وجود أخطاء في التوقيع
CLEAN_PRIVATE_KEY = (
    "-----BEGIN PRIVATE KEY-----\n"
...
# بيانات الـ Service Account مدمجة داخل الكود لتجنب مشاكل الملفات
...
def get_sheet_service():
    # استخدام from_service_account_info هو أضمن حل لمشكلة الـ PEM
...
def add_task_to_sheet(task_name):
    try:
        ...
        # الترتيب حسب شيت محمود الملون (Tasks)
        # A:المهمة | B:الأولوية | C:الحالة | D:تاريخ الإضافة | E:وقت البداية | F:الانهاء | G:المتوقع | H:الديدلاين | I:ملاحظات
        row_data = [
            task_name,          # Column A
            "متوسطة",            # Column B
            "قيد التنفيذ",       # Column C
            now_date,           # Column D
            now_time,           # Column E
            "", "", "",         # F, G, H
            "تمت الإضافة عبر البوت" # Column I
        ]
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    status = await update.message.reply_text("⏳ جاري الإضافة للشيت...")
...
def main():
    ...
    print("🚀 سكرتير محمود المصري شغال الآن...")
    app.run_polling()
